  ...
  return (
    <section id="blog" className="page-container mt-14">
      <div className="mb-7 flex items-end justify-between gap-5">
        ...
        <a className="hidden text-sm font-black text-[var(--green-dark)] sm:block" href="#blog" data-blog-all>
          Tüm Yazılar
        </a>
      </div>
      <div className="grid gap-5 lg:grid-cols-3">
        {posts.map((post) => (
          <article className="rounded-2xl border border-slate-200 bg-white p-6 soft-shadow" key={post.title} data-blog-card>
            ...
            <p className="mt-3 hidden rounded-xl bg-slate-50 p-4 text-sm leading-6 text-slate-600" data-blog-detail>
              Bu yazı önizlemesi, kullanıcıyı hesaplama yapmadan önce kritik sözleşme kalemlerini kontrol etmeye yönlendirir.
            </p>
            <a className="mt-5 inline-flex text-sm font-black text-[var(--green-dark)]" href="#blog" data-blog-read>
              Devamını Oku →
            </a>
          </article>
        ))}
