...
export function NewsletterSignup({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section className={compact ? "" : "page-container mt-14"}>
      <div className="rounded-[26px] border border-[#ffd8bd] bg-[linear-gradient(135deg,#fff4ea_0%,#ffffff_56%,#f6fbff_100%)] p-6 shadow-[0_18px_42px_rgba(77,48,31,0.06)] md:p-8">
        <div className="grid gap-5 lg:grid-cols-[1fr_420px] lg:items-center">
          <div>
            <span className="text-xs font-black uppercase tracking-[0.16em] text-[#a94612]">Bülten</span>
          ...
          <form className="grid gap-3 sm:grid-cols-[1fr_auto]" onSubmit={submit}>
            <input
              ...
              value={email}
            />
            <button className="rounded-[14px] bg-[#f47a2a] px-6 py-3 text-sm font-black text-white shadow-[0_14px_26px_rgba(244,122,42,0.18)]" type="submit">
              Kaydol
            </button>
            {message ? <p className="text-sm font-semibold text-[#a94612] sm:col-span-2">{message}</p> : null}
          </form>
        </div>
      </div>
    </section>
