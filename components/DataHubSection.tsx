...
function StatTile({
  ...
  const styles = {
    ...
    green: "border-[#caefdb] bg-[#ecfbf3] text-[#145c3b]",
  };
...
function HeroIndex() {
  ...
  return (
    <section className="page-container pt-8">
      <div className="overflow-hidden rounded-[28px] border border-[#dcefe4] bg-white shadow-[0_10px_32px_rgba(41,65,52,0.06)]">
        <div className="grid lg:grid-cols-[1.05fr_0.95fr]">
          <div className="relative min-h-[430px] min-w-0 p-7 md:min-h-[520px] md:p-10">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_left_top,rgba(104,250,195,0.16),transparent_22%),linear-gradient(180deg,#f8fffb_0%,#f7fbff_100%)]" />
            <div className="absolute inset-0 opacity-45 [background-image:radial-gradient(rgba(35,173,107,0.11)_1px,transparent_1px)] [background-size:18px_18px]" />

            <div className="relative z-10 flex h-full flex-col justify-between gap-10">
              <div>
                <span className="inline-flex max-w-full rounded-full border border-[#dff6e9] bg-[#f4fff8] px-4 py-2 text-center text-xs font-medium uppercase leading-5 tracking-[0.12em] text-[#20854f] md:tracking-[0.16em]">
                  Piyasada olmayan karar göstergesi
                </span>
                <h1 className="mt-8 max-w-[680px] break-words text-[clamp(34px,6vw,70px)] font-bold leading-[0.95] tracking-[-0.05em] text-[#0f1730]">
          ...
          <div className="relative min-w-0 p-7 md:p-10">
            <div className="mb-5 ml-auto w-fit rounded-full bg-[#e8fbef] px-4 py-2 text-sm font-medium text-[#20854f] lg:absolute lg:right-8 lg:top-8 lg:mb-0">
              {financingIndex.updatedLabel}
            </div>
...
function DriverBoard() {
  ...
  return (
    <section className="rounded-[26px] border border-[#dce7e2] bg-white p-6 shadow-[0_18px_48px_rgba(31,43,37,0.08)]">
...
function IndicatorRail() {
  ...
  return (
    <section className="rounded-[26px] border border-[#dce7e2] bg-white p-6 shadow-[0_18px_48px_rgba(31,43,37,0.08)]">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#179253]">Güncel veri paneli</p>
      <h2 className="mt-3 text-3xl font-bold tracking-[-0.05em] text-[#1c2433]">Canlı pano hissi, manuel kontrol.</h2>
        ...
        <div className="rounded-[26px] border border-[#dce7e2] bg-white p-6 shadow-[0_18px_48px_rgba(31,43,37,0.08)]">
          <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <h2 className="text-3xl font-bold tracking-[-0.05em] text-[#1c2433]">Son aylar aynı ekranda.</h2>
              <div className="mt-6 grid gap-3">
            ...
            <div className="rounded-[24px] border border-[#e6edf5] bg-[#f8fbfe] p-5">
      ...
      <div className="rounded-[26px] border border-[#dce7e2] bg-white p-6 shadow-[0_18px_48px_rgba(31,43,37,0.08)]">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="text-3xl font-bold tracking-[-0.05em] text-[#1c2433]">Veri nereden geliyor?</h2>
          </div>
          <span className="text-sm font-medium text-[#74829a]">{sourceCatalog.length} hat</span>
        </div>

        <div className="mt-6 grid max-h-[540px] gap-2 overflow-auto pr-2">
          {sourceCatalog.map((source, index) => (
            <a
              className="grid gap-3 rounded-2xl border border-[#e6edf5] bg-[#f8fbfe] p-4 transition hover:-translate-y-0.5 hover:border-[#caefdb] hover:bg-white hover:shadow-sm md:grid-cols-[38px_1fr_auto]"
              href={source.url || "#"}
              key={`${source.name}-${source.period}`}
              rel="noreferrer"
              target={source.url ? "_blank" : undefined}
            >
              <span className="text-sm font-medium text-[#95a2b3]">{String(index + 1).padStart(2, "0")}</span>
              <span>
                <strong className="block text-sm font-bold text-[#1c2433]">{source.name}</strong>
                <span className="mt-1 block text-sm leading-6 text-[#66768d]">{source.note}</span>
              </span>
              <span className="h-fit rounded-full bg-[#f5f8f6] px-3 py-1 text-xs font-medium uppercase tracking-[0.08em] text-[#74829a]">
