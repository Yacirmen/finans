...
function applyReferenceInspiredCopy() {
  const navLabels = [
    "Ana Sayfa",
    "Kredi Limit",
    "Manuel Analiz",
  ...
  if (heroKicker) heroKicker.textContent = "Banka Kredisi ile Karşılaştırmalı Analiz";
  if (heroTitle) {
    heroTitle.innerHTML = 'Tasarruf Finansmanının <span class="hero-green">Gerçek Maliyetini</span> Hesaplayın';
  }
  if (heroText) {
    heroText.textContent =
      "Enflasyondan arındırılmış net maliyet analizi, banka kredisi karşılaştırması ve peşinat optimizasyonu tek ekranda.";
  }

  const metricText = [
    ["Hesaplama", "Net maliyetinizi anında hesaplayın"],
    ["Karşılaştırma", "Teklifleri yan yana kıyaslayın"],
    ["Limit", "Azami kredi alanınızı görün"],
    ["Danışmanlık", "Senaryoyu karar diline çevirin"],
  ];
  document.querySelectorAll(".hero-metrics article").forEach((card, index) => {
    const [label, value] = metricText[index] || [];
